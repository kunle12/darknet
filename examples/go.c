...
float select_mcts(mcts_tree *root, network *net, float *prev, float cpuct) {
  ...
  int max_i = 0;
  for (i = 0; i < 19 * 19 + 1; ++i) {
    root->prob[i] = root->mean[i] + cpuct * root->prior[i] *
                                        sqrt(root->total_count) /
...
/*
   ...
   *ko, float temp)
   ...
   root->visit_count[max_i]); fprintf(stderr, "%d %d, Result: %f, Prior: %f,
   Prob: %f, Mean Value: %f, Child Result: %f, Visited: %d\n", ind/19, ind%19,
   root->result, root->prior[ind], root->prob[ind], root->mean[ind],
...
cpuct) int i, j; int empty = 1; for(i = 0; i < 19*19; ++i){ if (occupied(board,
i)) { empty = 0; break;
}
}
if(empty) {
...
void valid_go(char *cfgfile, char *weightfile, int multi, char *filename) {
  ...
  // moves m = load_go_moves("/home/kunle12/backup/go.test");
  moves m = load_go_moves(filename);
  ...
  while (1) {
    ...
    if (!strcmp(buff, "protocol_version")) {
      ...
      if (anon) {
        ...
        printf("=%s 1.0. Want more DarkGo? You can find me on OGS, unlimited "
