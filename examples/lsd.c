...
*acfg, char *aweight, int clear)
{
    ...
    //char *style_images = "/home/kunle12/data/coco/trainvalno5k.txt";
    char *style_images = "/home/kunle12/zelda.txt";
...
gnet->momentum, gnet->decay); int imgs = gnet->batch*gnet->subdivisions; int i =
*gnet->seen/imgs; data train, tbuffer; data style, sbuffer;
...
imlayer.outputs*imlayer.batch); for(k = 0; k < gnet->batch; ++k){ int index =
j*gnet->batch + k; copy_cpu(imlayer.outputs, imlayer.output + k*imlayer.outputs,
1, generated.X.vals[index], 1); generated.y.vals[index][0] = .1;
...
sec(clock()-time), i*imgs); if(i%1000==0){ char buff[256]; sprintf(buff,
"%s/%s_%d.weights", backup_directory, gbase, i); save_weights(gnet, buff);
...
clear)
{
    ...
    printf("Learning Rate: %g, Momentum: %g, Decay: %g\n", net->learning_rate,
net->momentum, net->decay); int imgs = net->batch*net->subdivisions; i =
*net->seen/imgs; data train, buffer;
...
cuda_pull_array(imlayer.output_gpu, imlayer.output,
imlayer.outputs*imlayer.batch); for(k = 0; k < net->batch; ++k){ int index =
j*net->batch + k; copy_cpu(imlayer.outputs, imlayer.output + k*imlayer.outputs,
1, gray.X.vals[index], 1); gray.y.vals[index][0] = .1;
}
}
harmless_update_network_gpu(anet);
...
sec(clock()-time), i*imgs); if(i%1000==0){ char buff[256]; sprintf(buff,
"%s/%s_%d.weights", backup_directory, base, i); save_weights(net, buff);
sprintf(buff, "%s/%s_%d.weights", backup_directory, abase, i);
save_weights(anet, buff);
}
if(i%100==0){
  ...
  while (get_current_batch(gnet) < maxbatch) {
    ...
    for (j = 0; j < gnet->subdivisions; ++j) {
      ...
      *gnet->seen += gnet->batch;
      ...
      // imlayer.outputs*imlayer.batch)); printf("features %f\n",
...
imlayer.outputs)); printf("realness %f\n", cuda_mag_array(imerror,
imlayer.outputs));
...
imlayer.outputs*imlayer.batch); for(k = 0; k < net->batch; ++k){ int index =
j*net->batch + k; copy_cpu(imlayer.outputs, imlayer.output + k*imlayer.outputs,
1, generated.X.vals[index], 1); generated.y.vals[index][0] = 0;
        }
    }
    harmless_update_network_gpu(anet);
...
get_current_rate(net), sec(clock()-time), i*imgs); if(i%1000==0){ char
buff[256]; sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i);
        save_weights(net, buff);
        sprintf(buff, "%s/%s_%d.weights", backup_directory, abase, i);
        save_weights(anet, buff);
    }
...
/*
   ...
   if(clear) *net->seen = 0;
   printf("Learning Rate: %g, Momentum: %g, Decay: %g\n", net->learning_rate,
net->momentum, net->decay); int imgs = net->batch*net->subdivisions; int i =
*net->seen/imgs; data train, buffer;
...
void run_lsd(int argc, char **argv) {
  ...
  char *file = find_char_arg(
      argc, argv, "-file", "/home/kunle12/data/imagenet/imagenet1k.train.list");
  ...
  // aweights, clear); else if(0==strcmp(argv[2], "traincolor"))
  // train_colorizer(cfg, weights, acfg, aweights, clear); else
  // if(0==strcmp(argv[2], "train3")) train_lsd3(argv[3], argv[4], argv[5],
