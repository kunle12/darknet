...
void delta_yolo_class(float *output, float *delta, int index, int class_idx,
                      int classes, int stride, float *avg_cat) {
  int n;
  if (delta[index]) {
    delta[index + stride * class_idx] = 1 - output[index + stride * class_idx];
    if (avg_cat)
      *avg_cat += output[index + stride * class_idx];
    return;
  }
  for (n = 0; n < classes; ++n) {
    delta[index + stride * n] =
        ((n == class_idx) ? 1 : 0) - output[index + stride * n];
    if (n == class_idx && avg_cat)
  ...
  for (b = 0; b < l.batch; ++b) {
      ...
      for (i = 0; i < l.w; ++i) {
        for (n = 0; n < l.n; ++n) {
          ...
          for (t = 0; t < l.max_boxes; ++t) {
            box truth =
                float_to_box(net->truth + t * (4 + 1) + b * l.truths, 1);
            if (!truth.x)
              break;
            float iou = box_iou(pred, truth);
            if (iou > best_iou) {
          ...
          if (best_iou > l.truth_thresh) {
            ...
            int class_idx = net->truth[best_t * (4 + 1) + b * l.truths + 4];
            if (l.map)
              class_idx = l.map[class_idx];
            int class_index =
                entry_index(l, b, n * l.w * l.h + j * l.w + i, 4 + 1);
            delta_yolo_class(l.output, l.delta, class_index, class_idx,
                             l.classes, l.w * l.h, 0);
            box truth =
    ...
    for (t = 0; t < l.max_boxes; ++t) {
      ...
      if (mask_n >= 0) {
        ...
        int class_idx = net->truth[t * (4 + 1) + b * l.truths + 4];
        if (l.map)
          class_idx = l.map[class_idx];
        int class_index =
            entry_index(l, b, mask_n * l.w * l.h + j * l.w + i, 4 + 1);
        delta_yolo_class(l.output, l.delta, class_index, class_idx, l.classes,
                         l.w * l.h, &avg_cat);
