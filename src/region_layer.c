...
void delta_region_class(float *output, float *delta, int index, int class_idx,
  ...
  if (hier) {
    ...
    while (class_idx >= 0) {
      ...
      delta[index + stride * class_idx] =
          scale * (1 - output[index + stride * class_idx]);
    ...
    *avg_cat += pred;
    ...
    for (n = 0; n < classes; ++n) {
      delta[index + stride * n] =
          scale * (((n == class_idx) ? 1 : 0) - output[index + stride * n]);
      if (n == class_idx)
  ...
  *(l.cost) = 0;
  for (b = 0; b < l.batch; ++b) {
    if (l.softmax_tree) {
      ...
      for (t = 0; t < 30; ++t) {
        ...
        if (!truth.x)
          break;
        int class_idx =
            net->truth[t * (l.coords + 1) + b * l.truths + l.coords];
        float maxp = 0;
        int maxi = 0;
        if (truth.x > 100000 && truth.y > 100000) {
          ...
          delta_region_class(l.output, l.delta, class_index, class_idx,
                             l.classes, l.softmax_tree, l.class_scale,
    ...
    for (t = 0; t < 30; ++t) {
      ...
      if (l.map)
        class_idx = l.map[class_idx];
      int class_index =
          entry_index(l, b, best_n * l.w * l.h + j * l.w + i, l.coords + 1);
      delta_region_class(l.output, l.delta, class_index, class_idx, l.classes,
