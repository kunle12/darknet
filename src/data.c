...
char **find_replace_paths(char **paths, int n, const char *find,
                          const char *replace) {
...
box bound_image(image im) {
  ...
  box b = {(float)minx, (float)miny, (float)(maxx - minx + 1),
           (float)(maxy - miny + 1)};
  // printf("%f %f %f %f\n", b.x, b.y, b.w, b.h);
  return b;
}
...
data load_data_captcha(char **paths, int n, int m, int k, int w, int h) {
  ...
  data d = {.w = 0,
            .h = 0,
            .X = {.rows = 0, .cols = 0, .vals = NULL},
            .y = {.rows = 0, .cols = 0, .vals = NULL},
            .shallow = 0,
            .num_boxes = NULL,
            .boxes = NULL};
