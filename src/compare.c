...
void bbox_update(sortable_bbox *a, sortable_bbox *b, int class_idx,
                 int result) {
  int k = 32;
  float EA =
      1. / (1 + pow(10, (b->elos[class_idx] - a->elos[class_idx]) / 400.));
  float EB =
      1. / (1 + pow(10, (a->elos[class_idx] - b->elos[class_idx]) / 400.));
  float SA = result ? 1 : 0;
  float SB = result ? 0 : 1;
