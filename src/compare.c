...
void bbox_update(sortable_bbox *a, sortable_bbox *b, int class_idx, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[class_idx] - a->elos[class_idx])/400.));
    float EB = 1./(1+pow(10, (a->elos[class_idx] - b->elos[class_idx])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[class_idx] += k*(SA - EA);
    b->elos[class_idx] += k*(SB - EB);
}

void bbox_fight(network * net, sortable_bbox *a, sortable_bbox *b, int classes, int class_idx)
{
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
  ...
  set_batch_network(net, 1);
   ...
   list *plist = get_paths((char*)"data/compare.sort.list");
    ...
    for (class_idx = 0; class_idx < classes; ++class_idx){
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, class_idx);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", class_idx);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[class_idx]);
        }
        fclose(outfp);
    }
